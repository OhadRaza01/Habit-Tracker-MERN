...
export default function WeeklyProgress({ points, completedCounts, dates, averageCompletion }) {
    return (
            ...
            <div className="px-6 pt-8">
                <div className="relative flex h-44 items-end justify-between gap-2 border-b border-dashed border-[#e8e1da] pb-0">
                    {points.map((point, index) => {
                        ...
                        const habitLabel = completedCount === 1 ? "habit" : "habits"

                        return <div key={`${dates[index]}-${point}`} className="group relative flex h-full flex-1 items-end justify-center" title={`${completedCount} ${habitLabel} completed on ${formatDate(dates[index])}`} aria-label={`${completedCount} ${habitLabel} completed on ${formatDate(dates[index])}`}><div className="absolute bottom-0 h-full w-px border-l border-dashed border-[#f1ede8]" /><div className="relative z-10 w-3 rounded-full border-[3px] border-white bg-[#f26b3a] shadow-[0_0_0_2px_#f26b3a]" style={{ height: `${point}%` }} /></div>
                    })}
                </div>
